...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction1,1020,1253,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(2);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1255,1672,N_PAR);
   fitFcn1.SetNpx(500);
   ...
   TF1 backFcn("backFcn",background,1035,1290,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,900,1450,N_PAR);
   ...
   TF1 backFcn1("backFcn1",background1,1080,1490,N_PAR);
   backFcn1.SetLineColor(kBlue);
   TF1 signalFcn1("signalFcn1",signal1,1000,1680,N_PAR);
...
double w=histo->GetBinWidth(1);
int integral1 = backFcn1.Integral(1250,1830)/w;
int integral2 = fitFcn1.Integral(1250,1780)/w;
...
void fitCo60WithoutADDBACK() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("subtracted60Cowithoutaddback.root");
