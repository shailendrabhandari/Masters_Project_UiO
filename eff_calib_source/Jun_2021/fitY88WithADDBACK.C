...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,650,1290,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(2);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1600,2250,N_PAR);
   fitFcn1.SetNpx(500);
...
int integral2= fitFcn1.Integral(1600,2100)/w;
...
void fitY88WithADDBACK() {
   ...
   float fitxmax=3000;
    ...
    TH1F *h = (TH1F*) f->Get("crystal_fired_doppler_addback_1D");   //Simulated
    //TH1F *h = (TH1F*) f->Get("doppler");
   // TH1F *h = (TH1F*) f->Get("hp_doppler_3"); //Experimental
   if (!h){
   ...
   FitRoutine(c1,h, fitxmin, fitxmax,"88Y_Jun_2021.png");
}  
