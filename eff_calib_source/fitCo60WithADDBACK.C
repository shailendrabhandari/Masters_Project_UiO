...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction1,940,1250,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(3);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1260,1680,N_PAR);
   fitFcn1.SetNpx(500);
   ...
   histo->GetXaxis()->SetRange(100,500);
   ...
   TF1 backFcn("backFcn",background,1010,1290,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,700,1450,N_PAR);
   ...
   TF1 backFcn1("backFcn1",background1,990,1500,N_PAR);
...
int integral1 = backFcn.Integral(1050,1250)/w;
int integral2 = fitFcn.Integral(940,1250)/w;
...
double w=histo->GetBinWidth(1);
int integral1 = backFcn1.Integral(1250,1630)/w;
int integral2 = fitFcn1.Integral(1260,1680)/w;
...
void fitCo60WithADDBACK() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("subtracted60Cowithaddback.root");
