...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,620,1250,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(2);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1630,2250,N_PAR);
   fitFcn1.SetNpx(500);
   ...
   TF1 backFcn1("backFcn1",background1,1670,2000,N_PAR);
...
int integral1 = backFcn.Integral(770,1250)/w;
int integral2= fitFcn.Integral(808.6,950)/w;
...
int integral2= fitFcn1.Integral(1711.5,2138.5)/w;
...
void fitY88WithoutADDBACK() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("subtracted88Ywithoutaddback.root");
   ...
   FitRoutine(c1,h, fitxmin, fitxmax,"88Yfitwithoutaddback.pdf");
}
