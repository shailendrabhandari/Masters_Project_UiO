...
void c_mult123(){
    ...
    sim[0]   = new TFile("77Zn_C_691_Reconstructor_MULT123.root"); 
    sim[1]   = new TFile("77Zn_C_887_Reconstructor_MULT123.root");
    sim[2]   = new TFile("77Zn_C_1170_Reconstructor_MULT123.root");
    sim[3]   = new TFile("77Zn_C_1409_Reconstructor_MULT123.root");
    sim[4]   = new TFile("77Zn_C_1936_Reconstructor_MULT123.root");
   ...
   // }
    ...
    hexp->GetXaxis()->SetRangeUser(150,6000);
    hexp->GetYaxis()->SetRangeUser(0,300);
    ...
    whole->SetParName(4,"691 keV");
    whole->SetParName(6,"887 keV");
    whole->SetParName(8,"1170 keV");
    whole->SetParName(10,"1409 keV");
    whole->SetParName(12,"1936 keV");
   // whole->SetParName(14,"2626 keV");
    ...
    Double_t value691 = norma1*0.022;
   
    Double_t norma2 = 0.074;
    Double_t value887 = norma2*0.017;



    Double_t norma3 = 0.10;
    Double_t value1170 = norma3*0.018;




    Double_t norma4 = 0.12;
    Double_t value1409 = norma4*0.016;


    Double_t norma5 = 0.16;
    Double_t value1936 = norma5*0.02;
   ...
   // Double_t value2626 = norma6*0.018;
    ...
    whole->FixParameter( 4, value691); //    691 peak 1
    whole->FixParameter( 5, 0.0);
    
    whole->FixParameter( 6, value887); //    887 peak 2
    whole->FixParameter( 7, 0.0);

    whole->FixParameter( 8, value1170); //   1170 peak 3
    whole->FixParameter( 9, 0.0);


    whole->FixParameter( 10, value1409); //  1409 peak 4
    whole->FixParameter( 13, 0.0);



    whole->FixParameter( 12, value1936); //  1936 peak 5
...
*/
   ...
   // Double_t chi2 = whole->GetChisquare(); ///whole->GetNDF();
    ...
    leg->AddEntry(hexp,"Experiment_C_Mult123 77Zn");
    leg->AddEntry(expon,"Double exponential BG");
    leg->AddEntry(peak0f,"GEANT4 Simulation 691 keV");
    leg->AddEntry(peak1f,"GEANT4 Simulation 887 keV");
    leg->AddEntry(peak2f,"GEANT4 Simulation 1170 keV");
    leg->AddEntry(peak3f,"GEANT4 Simulation 1409 keV");
    leg->AddEntry(peak4f,"GEANT4 Simulation 1936 keV");
   ...
   // leg->AddEntry(whole, Form("Chi2/NDF = %4.2f", chi2ndf), "l");
    ...
    fCanvas->Print("77Zn_mult123_no_AddBack.pdf");
}
