...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,560,860,N_PAR);
   fitFcn.SetNpx(500);
   ...
   TF1 backFcn("backFcn",background,560,770,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,450,780,N_PAR);
  ...
  //Integral of a function
    ...
    int integral1 = backFcn.Integral(550,750)/w;
    int integral2 = fitFcn.Integral(575,850)/w;
...
void withaddback_blacklistedfit() {
   ...
   FitRoutine(c1,h, fitxmin, fitxmax,"blecklisted_Addback_sim137Cs.pdf");
}
