...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,570,860,N_PAR);
   fitFcn.SetNpx(500);
   ...
   TF1 backFcn("backFcn",background,570,760,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,450,850,N_PAR);
  ...
  //Integral of a function
    ...
    int integral1 = backFcn.Integral(573,750)/w;
    int integral2 = fitFcn.Integral(470,860)/w;
...
void fit() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("137CsReconstructor.root");
   // TH1F *h = (TH1F*) f->Get("crystal_fired_doppler_addback_1D");   //withaddback
    TH1F *h = (TH1F*) f->Get("crystal_fired_doppler_blacklist");              //withoutaddback
   if (!h){
   ...
   FitRoutine(c1,h, fitxmin, fitxmax,"crystal_fired_doppler_blacklist.pdf");
}
