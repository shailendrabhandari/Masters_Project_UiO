...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,975,1250,N_PAR);
   fitFcn.SetNpx(500);
   ...
   histo->GetXaxis()->SetRange(60,550);
   histo->Fit("fitFcn","VR+","ep");
 
   // improve the picture:
   // Draw signal and background functions separately
   TF1 backFcn("backFcn",background,1015,1270,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,800,1250,N_PAR);
  ...
  //Integral of a function
    ...
    int integral2 = fitFcn.Integral(490,900)/w;
