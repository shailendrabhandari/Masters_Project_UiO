...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction,1075,1237,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(3);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1250,1810,N_PAR);
   fitFcn1.SetNpx(500);
   fitFcn1.SetLineWidth(3);
   ...
   histo->GetXaxis()->SetRange(140,500);
   ...
   TF1 backFcn("backFcn",background,1030,1290,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,700,1250,N_PAR);
   ...
   TF1 backFcn1("backFcn1",background1,1070,1550,N_PAR);
   backFcn1.SetLineColor(kBlue);
   TF1 signalFcn1("signalFcn1",signal1,900,1700,N_PAR);
...
int integral1 = backFcn.Integral(1065,1236)/w;
int integral2 = fitFcn.Integral(1065,1236)/w;
...
double w=histo->GetBinWidth(1);
int integral1 = backFcn1.Integral(1235,1550)/w;
int integral2 = fitFcn1.Integral(1235,1600)/w;
...
void fitCo60() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("/home/shailendra/geant4/new_efficiency/fitting/60Co/pureexp/c_60Co_-2000_2000_15_4Feb.root");
   TH1F *h = (TH1F*) f->Get("hp_doppler_0");           //withoutaddback
    //TH1F *h = (TH1F*) f->Get("hp_doppler_3");           // withaddback
   if (!h){
