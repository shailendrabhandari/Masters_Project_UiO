...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction1,795,1170,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(2);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1680,2150,N_PAR);
   fitFcn1.SetNpx(500);
   ...
   TF1 backFcn("backFcn",background,790,1000,N_PAR);
   backFcn.SetLineColor(kBlue);
   TF1 signalFcn("signalFcn",signal,630,1200,N_PAR);
   ...
   TF1 backFcn1("backFcn1",background1,1680,2000,N_PAR);
   backFcn1.SetLineColor(kBlue);
   TF1 signalFcn1("signalFcn1",signal1,1710,2100,N_PAR);
...
int integral1 = backFcn.Integral(770,1000)/w;
int integral2 = fitFcn.Integral(795,1170)/w;
...
////Area under 2nd peak
double w=histo->GetBinWidth(1);
int integral1 = backFcn1.Integral(1650,2100)/w;
int integral2= fitFcn1.Integral(1600,2300)/w;
...
cout<<"Integral of Fit1 function::::::"<<integral2<<endl;
   ...
   legend.AddEntry(histo,"G4simulated Data","lpe");
   legend.AddEntry(&backFcn1,"Background","l");
   legend.AddEntry(&signalFcn1,"Signal","l");
   legend.AddEntry(&fitFcn1,"Signal+Bkgd","l");
   legend.AddEntry(&fitFcn1,"Area under 1st peak=242044","l");
   legend.AddEntry(&fitFcn1,"Area under 2nd peak=164532","l");
   legend.DrawClone();
   histo->Draw("esame");
   c1->SaveAs(filename);*/
...
void with_addback_blacklisted_fitY88() {
   ...
   float fitxmax=3000;
    ...
    TFile* f = new TFile("88YReconstructor.root");
    TH1F *h = (TH1F*) f->Get("crystal_fired_doppler_addback_blacklist");   ///withaddback
   ...
   FitRoutine(c1,h, fitxmin, fitxmax,"88Yfit.pdf");
}
