...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 fitFcn("fitFcn",fitFunction1,720,1250,N_PAR);
   fitFcn.SetNpx(500);
   fitFcn.SetLineWidth(2);
   fitFcn.SetLineColor(kRed);


   TF1 fitFcn1("fitFcn1",fitFunction1,1650,2200,N_PAR);
   fitFcn1.SetNpx(500);
   ...
   TF1 backFcn("backFcn",background,780,1000,N_PAR);
...
int integral2 = fitFcn.Integral(750,1000)/w;
...
int integral2= fitFcn1.Integral(1650,2200)/w;
...
void fitY88() {
   ...
   float fitxmax=3000;
    ...
    TH1F *h = (TH1F*) f->Get("crystal_fired_doppler_addback_1D");   ///withaddback
